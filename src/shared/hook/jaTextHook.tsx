...
import { UnicodeContext, UnicodeRangeContext } from 'client';
...
function useHuri(){
  const kanjiRegex = useContext<UnicodeContextInterface>(UnicodeContext).kanji;
  const hiraganaRegex = useContext<UnicodeContextInterface>(UnicodeContext).hiragana;
  const kanjiStartRegex = useContext<UnicodeContextInterface>(UnicodeContext).kanjiStart;

  const yomiToHuri = (hyouki : string, yomi : string) => {
  ...
  const complexArr = (hyouki : string, yomi : string, offset : number) => {
    if(yomi == null){
      return [{
        data : hyouki,
        ruby : null,
...
function useJaText(){
  const unicodeRange = useContext<UnicodeRangeContextInterface>(UnicodeRangeContext);
  const unicodeContext = useContext<UnicodeContextInterface>(UnicodeContext);
  ...
  const koNFCToHira = ( hangul : string ) => {
  ...
  const hangulToHira = ( char : string, index : number, arr : Array<string> ) => {
    let normalized = char.normalize('NFD').replace(chosungsRegex, $0 => chosungs[$0.charCodeAt(0) - 0x1100]).replace(jungsungsRegex, $0 => junsungs[$0.charCodeAt(0) - 0x1161]).replace(jongsungsRegex, $0 => jongsungs[$0.charCodeAt(0) - 0x11A8]);
  ...
  const getRegexRevise = ( text : string ) => {
  ...
  const extractKanji = ( okuri : string ) => {
  ...
  const extractHira = ( okuri : string ) => {
    let hyouki_hira = okuri.match(isHiraRegex);

    return hyouki_hira;
  }

  const checkKatachi = ( nihongo : string ) => {
  ...
  const isOnajiOkuri = ( hyouki : string, yomi : string, newText : string ) => {
  ...
  const matchOkuri = (hyouki : string, yomi : string, bunText : string) : [string | null, number, number] => {
    let hyouki_type = checkKatachi(hyouki);
  ...
  const matchOkuriExec = function(hyouki : string, yomi : string, bunText : string){
  ...
  const traceHukumu = (hukumu : Array<Hukumu>, bunText : string, newText : string) => {
...
function useKirikae(value : string, handleChange : (e : React.ChangeEvent) => void){

  const [kirikae, setKirikae] = useState<string | null>(null);
  const [isKirikae, setIsKirikae] = useState<boolean>(false);

  const debounce = useDebounce();
  const debouncedKirikae = debounce( (value : boolean) => setIsKirikae( value ), 1000 );
...
function useMultiKirikae(multiValue : Array<string>, handleMultiChange : (e : React.ChangeEvent, index : number) => void ){
...
function useMultiInput(dependancy : string, defaultInput : string){
...
function useBun(){
  ...
  const honToDan = (honText : string) => {
    let danArr = honText.split('\n');

    return danArr;
  }

  const danToBun = (danText : string) => {
