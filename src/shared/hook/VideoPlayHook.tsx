...
interface AudioDataObj {
  right : Array<number>;
  left : Array<number>;
}

type setTimeFunction = (time : number | null) => void;

type SelectMarkerString = null | 'startTime' | 'endTime';

function useVideoPlayHook( target : React.RefObject<any>, filteredData : AudioDataObj, frame : number, markStart : () => void, markEnd : () => void, startTime : number, endTime : number, setStartTime : setTimeFunction, setEndTime : setTimeFunction, played : number, duration : number, playing : boolean, setPlaying : (playing : boolean) => void ){
  ...
  const [selectMarker, setSelectMarker] = useState<SelectMarkerString>(null);
  ...
  const getPrevAutoMarkerPoint = (time : number, range : number, threshold : number) => {
    /*
      일단 급하락 지점을 찾음.
      또한 서서히 상승은 무시
  ...
  const getNextAutoMarkerPoint = (time : number, range : number, threshold : number) => {
    /*
...
function useTimeStamp(){
  ...
  const floorFrame = (time : number, frameRate : number) => {
  ...
  const frameObj = function ( frame : number, frameRate : number ){
...
export { useVideoPlayHook, useTimeStamp }
