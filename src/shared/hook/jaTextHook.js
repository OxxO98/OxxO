...
import { UnicodeContext, UnicodeRangeContext } from 'client/MainContext.js';
...
function useHuri(){
  ...
  const yomiToHuri = (hyouki, yomi) => {
    ...
    for(let key in arrOkuri){
      let revise = endIndex + 1;
      // let revise = endIndex + arrKanji[key].length;
      let matchedYomi = yomi.indexOf(arrOkuri[key], revise);
      arrHuri.push( yomi.substring(endIndex, matchedYomi) );

      endIndex = matchedYomi + arrOkuri[key].length;
    ...
    if(yomi.substring(endIndex) != ''){
      ...
      if( arrOkuri != null){
        ...
        if( yomi.lastIndexOf(lastOkuri) == yomi.length - lastOkuri.length ){
          arrHuri[arrHuri.length-1] = yomi.substring(endIndex-lastOkuri, yomi.length - lastOkuri.length);
        }
        else{
...
function useJaText(){
  const unicodeRange = useContext(UnicodeRangeContext);
  const unicodeContext = useContext(UnicodeContext);

  const nfd = [...'각힣'.normalize('NFD')].map( el => el.charCodeAt(0) );
  const chosungsRegex = new RegExp(
    `[\\u${nfd[0].toString(16)}-\\u${nfd[3].toString(16)}]`,
    'g'
  );
  const jungsungsRegex = new RegExp(
    `[\\u${nfd[1].toString(16)}-\\u${nfd[4].toString(16)}]`,
    'g'
  );
  const jongsungsRegex = new RegExp(
    `[\\u${nfd[2].toString(16)}-\\u${nfd[5].toString(16)}]`,
    'g'
  );
  const isHangulRegex = new RegExp(
    `^[가-힣\-]+$`,
    'g'
  );
  const isAllNihongoRegex = new RegExp(
    `^[${unicodeRange.kanji}${unicodeRange.hiragana}${unicodeRange.katakana}]+$`,
    'g'
  );
  const isAllHiraRegex = new RegExp(
    `^[${unicodeRange.hiragana}]+$`
  );
  const isAllKanjiRegex = new RegExp(
    `^[${unicodeRange.kanji}]+$`
  );
  const isHiraRegex = new RegExp(
    `[${unicodeRange.hiragana}]+`,
    'g'
  )
  const isKanjiRegex = new RegExp(
    `[${unicodeRange.kanji}]+`,
    'g'
  )
  ...
  const koNFCToHira = ( hangul ) => {
    ...
    let hira = hangulArr.map( (el, index, arr) =>  hangulToHira(el, index, arr) )

    return hira.join('');
  }

  const checkChouon = ( fw ) => {
    let fwDe = fw.normalize('NFD').replace(chosungsRegex, $0 => chosungs[$0.charCodeAt(0) - 0x1100]).replace(jungsungsRegex, $0 => junsungs[$0.charCodeAt(0) - 0x1161]).replace(jongsungsRegex, $0 => jongsungs[$0.charCodeAt(0) - 0x11A8]);
    let fwSecond = hangulJunsungHiraMatch[ fwDe[1] ];
  ...
  const hangulToHira = ( el, index, arr ) => {
    let de = el.normalize('NFD').replace(chosungsRegex, $0 => chosungs[$0.charCodeAt(0) - 0x1100]).replace(jungsungsRegex, $0 => junsungs[$0.charCodeAt(0) - 0x1161]).replace(jongsungsRegex, $0 => jongsungs[$0.charCodeAt(0) - 0x11A8]);

    if(de.length >= 2 && de.length <= 3){
      //NFD NFC에 대해 좀더 생각, 'ㅋ'이랑 ㅋ과는 다른 듯. 뭔가 변환 생각해서 es-hangul 참조.
      let first = hangulChosungHiraMatch[ de[0] ];
      let second = hangulJunsungHiraMatch[ de[1] ];

      let third = ''
      if(de.length == 3){
        third = hangulJonsungHiraMatch[ de[2] ];
      }
  ...
  const getRegexRevise = ( text ) => {
    let kanjiArr = extractKanji(text).join('').split('');
  ...
  const checkKatachi = ( nihongo ) => {
    let isHiraKanjiRegex = new RegExp(
      `^[${unicodeRange.kanji}${unicodeRange.hiragana}]+$`,
      'g'
    );
  ...
  const isOnajiOkuri = ( hyouki, yomi, newText ) => {
    ...
    if(hyouki_type == 'kanji'){
      if(newText_type == 'okuri'){
        ...
        let exKanjiPattern = exKanji.map( (arr) => `(${arr}.*)` ).join('');
        let exKanjiRegex = new RegExp(
          `^(.*)${exKanjiPattern}$`
        );
        let exHiraPattern = exHira.map( (arr) => `(.*${arr})` ).join('');
      ...
      if(newText_type == 'kanji'){
        ...
        exKanjiRegex.lastIndex = 0;
        if( exKanjiRegex.test(newText) ){
          let maeOkuriPattern = exKanji.map( (arr) => `${arr}(?:.*)` ).join('');
        ...
        let exKanjiNew = extractKanji(newText).join('').split('');
        //console.log(exKanjiNew);
        let exHiraNew = extractHira(newText);

        let exKanjiNewPattern = exKanjiNew.map( (arr) => `(${arr}.*)` ).join('');
        let exKanjiNewRegex = new RegExp(
          `^(.*)${exKanjiNewPattern}$`
        );
        let exHiraNewPattern = exHiraNew.map( (arr) => `(.*${arr})` ).join('');
...
function useKirikae(value, handleChange){
  ...
  const [kirikae, setKirikae] = useState(null);
...
function useMultiKirikae(multiValue, handleMultiChange){
  ...
  // console.log('useKirikae', value, multiValue, isKirikae);
...
function useMultiInput(dependancy, defaultInput){
  ...
  const multiInput = (tango) => {
    ...
    for(let key in arrOkuri){
      ...
      if(tango.substring(endIndex, tango.indexOf(arrOkuri[key], endIndex)) == ''){
        startBool = false;
      }
      else{
        arrHuri.push(tango.substring(endIndex, tango.indexOf(arrOkuri[key], endIndex)));
      }
      endIndex = tango.indexOf(arrOkuri[key], endIndex)+arrOkuri[key].length;
...
export { useJaText, useHuri, useBun, useKirikae, useMultiKirikae, useMultiInput }
